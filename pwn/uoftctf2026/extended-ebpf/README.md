...
Keep in mind that when doing these checks the verifier **does not know** the actual contents of the registers, so for example if we execute an instruction `BPF_BPF_JMP_IMM(BPF_JGE, BPF_REG_1, 0, 20) (jump 20 instructions forward if r1 > 2)`, and the branch is not taken, the state will be updated to a new range, thinking that r1 is now ``(0, 1)`` (in the unsigned ranges). This is done to prevent OOB accesses, there are more rules but for now this is what we need to understand.

After passing these checks, there is another security measure called **ALU Sanitation**, this will patch the actual eBPF bytecode being executed to check if the state of the verifier matches with the actual state in runtime.
